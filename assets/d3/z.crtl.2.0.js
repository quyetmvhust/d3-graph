...
function buttonElement(d,i){
	...
	backbox.attr("class","button-background")
		...
		.on("click",function(){
			...
			d3.select(this)
				.transition().attr("class","button-background-lit")
				.transition().attr("class","button-background")
...
function radioElement(d,i){
	
	var hakenschniepel = document.createElementNS("http://www.w3.org/2000/svg", "g");
	var N = d.choices().length;
	var n = d3.range(N);
	var b = new block([N],d.size(),0,"[]");
	...
	var button = checkbox.selectAll(".radiobutton").data(n).enter().append("g")
		...
		.attr("transform",function(x,j){return "translate(0,"+b.x(j)+")"})
	...
	button.append("text")
		...
		.attr("transform","translate("+(d.buttonsize()/2+d.padding())+",0)")
...
function toggle(p){
	var parameter = p,
		...
		trackBorder = 0.5,
		labeled = true,
		update = function(x){console.log(x.value())};
		...
		this.update = function(a) { if ("function" === typeof a) {arg = a; return this} else { update(a) }};
}
...
function block(blocks,size,gap,t){
	var type;
	var x0;
	var n = blocks.length;
	var N = d3.sum(blocks);
